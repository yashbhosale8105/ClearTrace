...
export default function Investigate() {
    ...
    useEffect(() => {
        ...
        fetch(`/api/investigate/customer/${custId}`)
            .then(res => res.json())
            .then(result => {
                if (result.error) {
                ...
                if (result && result.customer) {
                    setEmailInput(result.customer.email || '');
                }
                setLoading(false);
            })
            .catch(err => {
    ...
    const downloadReport = async () => {
        try {
            const res = await fetch(`/api/generate_customer_report/${custId}`);
    ...
    const sendEmail = async () => {
        ...
        try {
            const r = await fetch('/api/send_customer_alert', {
                method: 'POST',
            ...
            if (result.success) {
                ...
                setEmailStatus({ type: 'error', msg: `❌ ${result.error || "Failed to send"}` });
            }
        } catch (e) {
