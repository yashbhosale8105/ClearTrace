...
# ── Gemini Setup ──
GEMINI_API_KEY = "AIzaSyDap917uGjpT0mVlLrmD-N0yxA3osiONCg"
gemini_model = None
...
# Simple cache for reports to speed up repeat views
REPORT_CACHE = {}
...
def api_chatbot_query():
    ...
    if not match_cust.empty:
        for _, row in match_cust.head(3).iterrows():
            ...
            context += f"- Recent Activity: {recent[['transaction_uuid', 'transaction_amount', 'transaction_status']].to_dict('records')}\n"
    ...
    for tid in txn_ids:
        ...
        if not t_row.empty:
            t_data = t_row.iloc[0].to_dict()
            found_entities.append(f"Transaction: {tid}")
            context += f"\nDetailed Data for Transaction {tid}: {t_data}\n"
    ...
    for cid in chq_ids:
        if cheques_df is not None:
            ...
            if not c_row.empty:
                c_data = c_row.iloc[0].to_dict()
                found_entities.append(f"Cheque: {cid}")
                context += f"\nData for Cheque {cid}: {c_data}\n"
    ...
    if cheques_df is not None:
        ...
        if not match_chq.empty:
            for _, row in match_chq.head(3).iterrows():
                ...
                context += f"\nCheque {row['cheque_number']} Details: {row.to_dict()}\n"
    ...
    full_prompt = (
        ...
        "If a cheque is 'is_tampered' = True, highlight the 'tamper_type' and explain the risk."
    )
